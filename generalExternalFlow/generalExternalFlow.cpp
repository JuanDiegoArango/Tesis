...
void updateMovingSurfaces(SimulationParameters &param, plint iIter,Array<T,3> force,double *velocidad)
{
    for (plint iMovingSurface = 0; iMovingSurface < param.numMovingSurfaces; iMovingSurface++) {
        ...
        for (plint iVertex = 0; iVertex < numVertices; iVertex++) {
            ...
            position[0] = position[0]+velocidad[0]*param.dt+(force[0]*tiempo_cuadrado)/(2*m);
            position[1] = position[1]+velocidad[1]*param.dt+(force[1]*tiempo_cuadrado)/(2*m);
            position[2] = position[2]+velocidad[2]*param.dt+(force[2]*tiempo_cuadrado)/(2*m);
            
            velocidad[0]=(param.dt*force[0]/m)+velocidad[0];
            velocidad[1]=(param.dt*force[1]/m)+velocidad[1];
            velocidad[2]=(param.dt*force[2]/m)+velocidad[2];
...
int main(int argc, char* argv[])
{
    ...
    const int MAX = 3;
     ...
     double *velocidad=new double[MAX];
    ...
    double v=0;
    for (int i=1;i<param.nz-1;i++)
    {
        Box3D a=Box3D(0,      param.nx-1,   0,      param.ny-1,   i ,  i);
        v=i*(param.inletVelocity_LB[0])/(param.nz-1);
        setBoundaryVelocity(*lattice, a, Array<T,3>(v,0.,0.));
    }
    ...
    for (plint iIter = iniIter; iIter < param.maxIter && !stopExecution; iIter++) {
        ...
        updateMovingSurfaces(param, param.nextIter,force,velocidad);
